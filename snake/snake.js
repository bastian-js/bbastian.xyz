window.addEventListener("DOMContentLoaded", function (event) {
    ...
    window.addEventListener("keydown", function (event) {
      ...
      if (event.key == "H" || event.key == "h") {
        ...
        noteElement.innerHTML = `Willkommen im schweren Modus. Drücke die Leertaste um das Spiel zu starten!`;
      ...
      if (event.key == "E" || event.key == "e") {
        ...
        noteElement.innerHTML = `Willkommen im leichten Modus. Drücke die Leertaste um das Spiel zu starten.`;
        noteElement.style.opacity = 1;
        resetGame();
        return;
      }
    ...
    function main(timestamp) {
      try {
        ...
        const pressSpaceToStart = "Drücke die Leertaste, um das Spiel neuzustarten!";
        const changeMode = hardMode
          ? "Du willst zurück in den leichten Modus? Drücke 'E'"
          : "Du willst in den schweren Modus? Drücke 'H'";
        const followMe =
          'Follow me <a href="https://twitter.com/smolcockbastian" , target="_blank">@smolcockbastian</a>';
    ...
    function getNextPosition() {
      ...
      switch (snakeDirection) {
        case "right": {
          ...
          if (nextPosition % width == 0) throw Error("Leider verloren :( Die Schlange ist gegen eine Wand gefahren!");
          // Ignore the last snake part, it'll move out as the head moves in
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("Leider verloren :( Die Schlange hat sich selbst gebissen!");
          return nextPosition;
        }
        case "left": {
          const nextPosition = headPosition - 1;
          if (nextPosition % width == width - 1 || nextPosition < 0)
            throw Error("Leider verloren :( Die Schlange ist gegen eine Wand gefahren!");
          // Ignore the last snake part, it'll move out as the head moves in
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("Leider verloren :( Die Schlange hat sich selbst gebissen!");
          return nextPosition;
        ...
        case "up": {
          const nextPosition = headPosition - width;
          if (nextPosition < 0) throw Error("Leider verloren :( Die Schlange ist gegen eine Wand gefahren!");
          // Ignore the last snake part, it'll move out as the head moves in
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("Leider verloren :( Die Schlange hat sich selbst gebissen!");
          return nextPosition;
